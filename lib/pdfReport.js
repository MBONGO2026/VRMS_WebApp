...
const { SERIES, TRACK } = require('./chartPalette');

const C = {
  brand: '#4A3223', brandDark: '#35231A', orange: '#B8520F', bg: '#F6F2EE', border: '#E6DDD3',
  text: '#22283A', muted: '#6B7280', green: '#1E7E34', red: '#B02A2A', blue: '#2E75B6', teal: '#17A2B8',
  zebra: '#F8F4F0', grid: '#E9E2DA',
};
const MARGIN = 40;
...
function banner(L, title, subtitle) {
  const { doc } = L;
  doc.rect(0, 0, doc.page.width, 64).fill(C.brand);
  doc.rect(0, 64, doc.page.width, 20).fill(C.brandDark);
...
function sectionTitle(L, title) {
  L.ensure(40);
  L.y += 6;
  L.text(title, L.x, L.y, { font: 'Helvetica-Bold', size: 11.5, color: C.brand });
...
function kpiCards(L, cards) {
  ...
  for (let i = 0; i < cards.length; i += perRow) {
    ...
    cards.slice(i, i + perRow).forEach((c, j) => {
      ...
      L.doc.rect(x, L.y, 3.5, h).fill(C.brand);
      // The stripe carries the card's accent; label and value stay in text ink.
      L.text(c.label, x + 12, L.y + 9, { font: 'Helvetica-Bold', size: 7.5, color: C.muted, width: cw - 18, ellipsis: true });
      L.text(c.value, x + 12, L.y + 20, { font: 'Helvetica-Bold', size: 16, color: C.text, width: cw - 18, ellipsis: true });
...
const BAR_MAX = 18; // pt: thin bars, the rest of each band stays empty
const GAP = 2; // pt of white between touching marks (stack segments, side-by-side bars)
const RADIUS = 3; // pt: rounded data end, square where the bar meets its baseline

// A column rising from `base` (rounded top), or a bar growing right from x (rounded end).
function columnMark(doc, x, top, w, h, color, rounded = true) {
  if (h <= 0 || w <= 0) return;
  const r = rounded ? Math.min(RADIUS, w / 2, h) : 0;
  if (!r) { doc.rect(x, top, w, h).fill(color); return; }
  doc.roundedRect(x, top, w, h, r).fill(color);
  doc.rect(x, top + r, w, h - r).fill(color);
}
function barMark(doc, x, y, w, h, color) {
  if (w <= 0 || h <= 0) return;
  const r = Math.min(RADIUS, h / 2, w);
  doc.roundedRect(x, y, w, h, r).fill(color);
  doc.rect(x, y, w - r, h).fill(color);
}
...
function chartFrame(L, x, y, w, h, title, series) {
  ...
  L.text(title, x, y + 9, { font: 'Helvetica-Bold', size: 9.5, color: C.brand, width: w, align: 'center' });
  if (series.length > 1) {
    let lx = x + 12;
    series.forEach((s) => {
...
function columnChart(L, x, y, w, h, { title, categories, series, stacked = false, fmt = (v) => v, integer = false }) {
  ...
  for (let k = 0; k <= steps; k++) {
    ...
    doc.moveTo(px, gy).lineTo(px + pw, gy).lineWidth(0.5).strokeColor(k === 0 ? '#C8BCB0' : C.grid).stroke();
  ...
  categories.forEach((cat, i) => {
    const gx = px + i * gw;
    if (stacked) {
      const bw = Math.min(gw * 0.5, BAR_MAX);
      const bx = gx + (gw - bw) / 2;
      const filled = series.map((s) => num(s.values[i])).map((v, j) => ({ v, j })).filter((p) => p.v > 0);
      let base = py + ph;
      filled.forEach(({ v, j }, k) => {
        const isTop = k === filled.length - 1;
        // Every segment above the first gives up GAP pt of white to separate it from the one below.
        const bh = (v / max) * ph - (k > 0 ? GAP : 0);
        if (k > 0) base -= GAP;
        columnMark(doc, bx, base - bh, bw, bh, series[j].color, isTop);
        base -= bh;
      });
      if (showValues && totals[i] > 0) L.text(fmt(totals[i]), gx, base - 10, { size: 6.5, color: C.text, width: gw, align: 'center' });
    } else {
      const bw = Math.min((gw * 0.6 - GAP * (series.length - 1)) / series.length, BAR_MAX);
      const start = gx + (gw - (bw * series.length + GAP * (series.length - 1))) / 2;
      series.forEach((s, j) => {
        const v = num(s.values[i]);
        const bh = (v / max) * ph;
        const bx = start + j * (bw + GAP);
        columnMark(doc, bx, py + ph - bh, bw, bh, s.color);
        if (showValues && series.length === 1) L.text(fmt(v), bx - 10, py + ph - bh - 10, { size: 6.5, color: C.text, width: bw + 20, align: 'center' });
      });
    }
    if (i % labelEvery === 0) {
...
function ratioChart(L, x, y, w, h, { title, categories, values, color, fmt }) {
  const { doc } = L;
  ...
  categories.forEach((cat, i) => {
    ...
    const bh = Math.min(rowH * 0.5, 10);
    const by = ry + (rowH - bh) / 2;
    doc.roundedRect(px, by, pw, bh, Math.min(RADIUS, bh / 2)).fill(TRACK);
    barMark(doc, px, by, pw * v, bh, color);
    L.text(fmt(values[i]), px + pw + 5, ry + rowH / 2 - 4, { font: 'Helvetica-Bold', size: 7.5 });
  });
}
...
function table(L, { title, columns, rows, emptyText, totals }) {
  ...
  if (title) {
    // Keep the heading with its table: room for the title, the header and two rows (or the empty message).
    L.ensure(40 + (rows.length ? headH + rowH * 2 : 24));
  ...
  const header = () => {
    doc.rect(L.x, L.y, L.w, headH).fill(C.brand);
    let cx = L.x;
    cols.forEach((c) => {
      L.text(L.fit(c.header, c.w - 10, 'Helvetica-Bold', 7.2), cx + 5, L.y + 6.5, { font: 'Helvetica-Bold', size: 7.2, color: '#FFFFFF', width: c.w - 10, align: c.align || 'left' });
      cx += c.w;
    });
    L.y += headH;
  };
  ...
  if (totals) {
    if (L.y + rowH > L.bottom) { L.newPage(); header(); }
    line(totals, { bold: true, fill: '#EFE5DB' });
  }
  L.y += 12;
}
...
function formatters(lang) {
  ...
  return {
    ...
    pct: (v) => (v == null ? '-' : num(v).toLocaleString(locale, { style: 'percent', minimumFractionDigits: 1, maximumFractionDigits: 1 })),
  };
}
...
function buildOverviewPdf(data, { t, lang, generatedAt = new Date() }) {
  ...
  return render(title, (doc, L) => {
    ...
    kpiCards(L, [
      { label: t('reports.colTotalRevenue'), value: f.money0(revenue), caption: t('xlsx.kpiInvoices', { n: sum(data.revenue, 'invoicesissued') }), color: C.brand },
      { label: t('reports.colCollected'), value: f.money0(collected), caption: t('xlsx.kpiCollectionRate', { pct: (revenue ? (collected / revenue) * 100 : 0).toLocaleString(f.locale, { minimumFractionDigits: 1, maximumFractionDigits: 1 }) }), color: C.green },
    ...
    chartRow(L, [
      (x, y, w, h) => columnChart(L, x, y, w, h, {
        ...
        series: [
          { name: t('reports.colCollected'), values: data.revenue.map((r) => r.totalcollected), color: SERIES.blue },
          { name: t('reports.colOutstanding'), values: data.revenue.map((r) => r.totaloutstanding), color: SERIES.orange },
        ],
      }),
      (x, y, w, h) => columnChart(L, x, y, w, h, {
        title: t('reports.bookingsByCategory'), integer: true, fmt: (v) => Math.round(v),
        categories: data.byCategory.map((r) => r.categoryname),
        series: [{ name: t('reports.colTotalBookings'), values: data.byCategory.map((r) => r.totalbookings), color: SERIES.blue }],
      }),
    ]);
    chartRow(L, [
      (x, y, w, h) => ratioChart(L, x, y, w, h, {
        title: t('xlsx.chartUtilization'), color: SERIES.blue, fmt: f.pct,
    ...
    table(L, {
      ...
      columns: [
        ...
        { header: t('reports.colUtilization'), align: 'right', value: (r) => f.pct(num(r.utilizationpercent) / 100) },
      ],
    });
    table(L, {
...
function buildPeriodPdf(data, period, { t, lang, generatedAt = new Date() }) {
  ...
  return render(`${title} - ${period.label}`, (doc, L) => {
    ...
    kpiCards(L, [
      ...
      { label: t('period.kpiInvoiced'), value: f.money0(s.invoiced), caption: t('xlsx.kpiInvoices', { n: s.invoices }), color: C.brand },
    ...
    if (data.trend.length) {
      chartRow(L, [(x, y, w, h) => columnChart(L, x, y, w, h, {
        ...
        series: [
          { name: t('period.kpiInvoiced'), values: data.trend.map((b) => b.invoiced), color: SERIES.orange },
          { name: t('period.kpiReceived'), values: data.trend.map((b) => b.received), color: SERIES.blue },
        ],
      })], 150);
    }
    chartRow(L, [
      (x, y, w, h) => columnChart(L, x, y, w, h, {
        ...
        categories: data.revenue.map((r) => r.categoryname),
        series: [
          { name: t('reports.colCollected'), values: data.revenue.map((r) => r.collected), color: SERIES.blue },
          { name: t('reports.colOutstanding'), values: data.revenue.map((r) => r.outstanding), color: SERIES.orange },
        ],
      }),
      (x, y, w, h) => columnChart(L, x, y, w, h, {
        title: t('reports.bookingsByCategory'), integer: true, fmt: (v) => Math.round(v),
        categories: data.bookingsByCategory.map((r) => r.categoryname),
        series: [{ name: t('period.kpiBookings'), values: data.bookingsByCategory.map((r) => r.bookings), color: SERIES.blue }],
      }),
    ], 170);
    chartRow(L, [
      (x, y, w, h) => ratioChart(L, x, y, w, h, {
        title: t('period.utilizationByCategory'), color: SERIES.blue, fmt: f.pct,
      ...
      (x, y, w, h) => columnChart(L, x, y, w, h, {
        ...
        series: [{ name: t('period.kpiInvoiced'), values: data.branches.map((r) => r.invoiced), color: SERIES.blue }],
      }),
    ], 150);
    ...
    table(L, {
      ...
      columns: [
        ...
        { header: t('period.colMethod'), width: 1.1, value: (r) => statusLabel(lang, 'paymentmethod', r.paymentmethod) },
