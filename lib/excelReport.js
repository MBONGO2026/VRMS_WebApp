...
async function buildPeriodExcelReport(data, period, { t, lang, generatedAt = new Date() }) {
  ...
  if (hasTrend) {
    charts.push({
      ...
      series: [
        { name: t('period.kpiInvoiced'), ref: trend.range('inv'), values: data.trend.map((r) => num(r.invoiced)), color: C.navy },
        { name: t('period.kpiReceived'), ref: trend.range('rec'), values: data.trend.map((r) => num(r.received)), color: C.green },
      ],
    });
  }
  charts.push({
    type: 'bar', anchor: [1, r0, 7, r0 + 16], dir: 'col', grouping: 'stacked',
    title: t('reports.revenueByCategory'), numFmt: MONEY0, labelFmt: '#,##0;-#,##0;;',
    categories: { ref: revenue.range('cat'), values: labels(data.revenue, 'categoryname') },
    series: [
      { name: t('reports.colCollected'), ref: revenue.range('col'), values: data.revenue.map((r) => num(r.collected)), color: C.green },
      { name: t('reports.colOutstanding'), ref: revenue.range('out'), values: data.revenue.map((r) => num(r.outstanding)), color: C.red },
    ],
  });
  charts.push({
    type: 'bar', anchor: [7, r0, 13, r0 + 16], dir: 'col', legend: false, gapWidth: 50, majorUnit: countStep(data.bookingsByCategory, 'bookings'),
    ...
    series: [{ name: t('period.kpiBookings'), ref: byCat.range('n'), values: data.bookingsByCategory.map((r) => num(r.bookings)), color: C.navy }],
  });
  charts.push({
    type: 'bar', anchor: [1, r0 + 17, 7, r0 + 33], dir: 'bar', legend: false, max: 1, gapWidth: 50,
    title: t('period.utilizationByCategory'), numFmt: '0%',
    categories: { ref: util.range('cat'), values: labels(data.utilization, 'categoryname') },
    series: [{ name: t('reports.colUtilization'), ref: util.range('util'), values: data.utilization.map((r) => r.utilization ?? 0), color: C.orange }],
  });
  charts.push({
    type: 'bar', anchor: [7, r0 + 17, 13, r0 + 33], dir: 'bar', legend: false, gapWidth: 50,
    title: t('period.byBranch'), numFmt: MONEY0,
    categories: { ref: branches.range('br'), values: labels(data.branches, 'branchname') },
    series: [{ name: t('period.kpiInvoiced'), ref: branches.range('inv'), values: data.branches.map((r) => num(r.invoiced)), color: C.blue }],
  });
