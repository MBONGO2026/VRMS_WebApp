...
const C = {
  brand: '4A3223', brandDark: '35231A', orange: 'B8520F', bg: 'F6F2EE',
  border: 'E6DDD3', text: '22283A', muted: '62584F', green: '1E7E34', red: 'B02A2A', amber: 'B8860B',
...
function addDataSheet(wb, { name, title, subtitle, tableName, columns, rows, emptyText, t }) {
  ...
  if (rows.length) {
    ...
    columns.forEach((c, i) => { ws.getCell(TABLE_HEADER_ROW, i + 1).fill = solid(C.brand); });
...
function sectionTitle(ws, row, col, endCol, text) {
  ...
  c.value = text;
  c.font = { name: 'Segoe UI', size: 11, bold: true, color: argb(C.brand) };
...
async function buildExcelReport(data, { t, lang, generatedAt = new Date() }) {
  ...
  kpiCard(dash, 2, {
    label: t('reports.colTotalRevenue'), color: C.brand, numFmt: MONEY0,
  ...
  const bodyStyle = (cell, zebra, horizontal = 'left') => {
    ...
    cell.fill = solid(zebra ? 'F6F0EA' : 'FFFFFF');
...
async function buildPeriodExcelReport(data, period, { t, lang, generatedAt = new Date() }) {
  ...
  const payments = addDataSheet(wb, {
    ...
    columns: [
      ...
      { key: 'meth', header: t('period.colMethod'), width: 18, value: (r) => statusLabel(lang, 'paymentmethod', r.paymentmethod) },
  ...
  kpiCard(dash, 8, {
    label: t('period.kpiInvoiced'), color: C.brand, numFmt: MONEY0,
